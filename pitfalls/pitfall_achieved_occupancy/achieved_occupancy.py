...
def main(mode, num_tb, num_threads_per_tb, num_iters_comp):
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 5:
        print("Usage: python script.py <mode> <num_tb> <num_threads_per_tb> <num_iters_comp>")
        sys.exit(1)

    mode = int(sys.argv[1])
    num_tb = int(sys.argv[2])
    num_threads_per_tb = int(sys.argv[3])
    num_iters_comp = int(sys.argv[4])

    main(mode, num_tb, num_threads_per_tb, num_iters_comp)
