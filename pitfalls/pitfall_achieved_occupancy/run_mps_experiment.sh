...
python achieved_occupancy.py 1 $num_tb $num_threads_per_tb $num_itrs

# measure sequential latency of two compute kernels on all SMs WITHOUT MPS
python achieved_occupancy.py 2 $num_tb $num_threads_per_tb $num_itrs
...
CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$achieved_occupancy python achieved_occupancy.py 1 $num_tb $num_threads_per_tb $num_itrs &
CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$achieved_occupancy python achieved_occupancy.py 1 $num_tb $num_threads_per_tb $num_itrs
