...
python comp_mem_ipc.py 1 $NUM_THREADS_PER_BLOCK $NUM_ITERS_COMP_BENCH $NUM_ITERS_MEM_BENCH

# measure latency of compute and copy kernel when running concurrently
python comp_mem_ipc.py 3 $NUM_THREADS_PER_BLOCK $NUM_ITERS_COMP_BENCH $NUM_ITERS_MEM_BENCH
