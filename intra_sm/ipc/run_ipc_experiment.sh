...
python3 ipc.py 1 0 $threads_per_tb_copy $threads_per_tb_comp $num_itrs_copy $num_itrs_comp $num_bytes

# Measure the latency of a single compute kernel
python3 ipc.py 1 $ILP $threads_per_tb_copy $threads_per_tb_comp $num_itrs_copy $num_itrs_comp $num_bytes
...
python3 ipc.py 2 $ILP $threads_per_tb_copy $threads_per_tb_comp $num_itrs_copy $num_itrs_comp $num_bytes

# Measure the latency of a copy and compute kernel running concurrently
python3 ipc.py 3 $ILP $threads_per_tb_copy $threads_per_tb_comp $num_itrs_copy $num_itrs_comp $num_bytes